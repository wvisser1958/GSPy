...
class TCombustor(gaspath):        
    ...
    def Run(self, Mode, PointTime, GasIn: ct.Quantity, Ambient) -> ct.Quantity:    
        ...
        if Mode == 'DP':
            self.w_fuel = self.Wfdes
        else:
            self.w_fuel = self.Control.Wf
        T_fuel = 288.15
        Sin = GasIn.s
        Pin = GasIn.P
        ...
        O2_exit_mass = w_air * fg.air_O2_fraction_mass - self.w_fuel/CHyOzMoleMass * (1+HC_ratio/4) * fg.O2_molar_mass 
        CO2_exit_mass = fg.CO2_molar_mass * self.w_fuel/CHyOzMoleMass + w_air*fg.air_CO2_fraction_mass   
        H2O_exit_mass = fg.H2O_molar_mass * self.w_fuel/CHyOzMoleMass * HC_ratio/2
        ...
        h_prod_final = (self.w_fuel * LHV_kJ_kg * 1000 + w_air * (h_air_initial-fg.h_air_ref)) / (w_air + self.w_fuel) + h_prod_ref

        self.GasOut.HP = h_prod_final, Pout
        self.GasOut.mass = self.GasIn.mass + self.w_fuel
