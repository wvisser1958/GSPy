...
# Modified by Lucas Middendorp, Oscar Kogenhop
...
from gspy.extensions.adaptive_modelling.AMcontrol_LM import TAMcontrol
...
def main():
    ...
    AMcontrol = TAMcontrol(turbojet, 'AMcontrol',
                        ...
                        [['T5', 1.0],   # note: tolerances are multipliers of the residuals (error equations), standard set to 1.0
                         ...
                         ['T3', 1.0],
                         ['N1%', 1.0]],
                        [1e2],      # bounds penalty multiplier: is multiplied with the error squared if a map modifier goes out of bounds
                        [
                            [(compressor1.map, "SF_eta_deter"), [None, None]], # note: bounds are percentage change from 100%
                            [(compressor1.map, "SF_wc_deter"), [None, None]],
                            [(turbine1.map, "SF_eta_deter"), [None, None]],
                            [(turbine1.map, "SF_wc_deter"), [None, None]]
                        ])


    # create a turbojet system model
    turbojet.define_comp_run_list(AMcontrol,  # AMcontrol first as it also reads data to set the Ambient conditions....
                                  inlet1,
                                  compressor1,
                                  combustor1,
                                  turbine1,
