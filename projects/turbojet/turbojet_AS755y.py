...
# Uncomment control creation statement for either fuel flow ("Fcontrol"), N1% ("Ncontrol") or EGT aka T5 ("EGTcontrol"):
    ...
    inlet1   = TInlet('Inlet1',      '', None,           '000','020',   19.9, 1    )

    compressor1 = TCompressor('compressor1',map_path / 'compmap.map' , None, '020', '030', 1, 16540, 0.825, 1, 0.75   , 6.92, 'GG', None)
    # option for polytropic efficiency, uncomment next line
    # compressor1.Polytropic_Eta = 1

    # OD fuel input from FuelControl
    combustor1 = TCombustor('combustor1', '',  FuelControl, '030', '040', 0.38, None, 1, 1, None,      43031, 1.9167, 0, None, None)
    # Texit input
    ...
    turbine1 =    TTurbine(   'turbine1'   , map_path / 'turbimap.map', None, '040', '050', 1, 16540, 0.88 , 1, 0.50943, 0.99, 'GG', None)
    # option for polytropic efficiency, uncomment next line
    # turbine1.Polytropic_Eta = 1

    duct1    = TDuct('exhduct',      '', None,            '050','070',   1.0        )
    exhaustnozzle = TExhaustNozzle('exhaustnozzle',  '', None,            '070','080','090', 1, 1, 1)
