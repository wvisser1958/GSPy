...
def main():
    ...
    fuel_control = TControl(turboshaft_2sp, 'Control', '', 2.5, 2.5, 1.8, -0.1, None)
    ...
    compressor1bleeds = [TBleedFlow(turboshaft_2sp, 'IPbleed', None, None,
                                    ...
                                    1,          # bleed number
                                    0.04,       # fraction of compressor inlet flow
                                    0.6),       # dP factor: bleed extraction pressure fraction: Pbleed = Pin + dP * dPtotal
    ...
    compressor1 = TCompressor(turboshaft_2sp, 'compressor1', 'compmap.map' , None, 2, 3, '_gg', 6800, 0.9, 1, 0.79   , 18, 'GG', compressor1bleeds)
    ...
    GGTcoolingflows = [TCoolingFlow(turboshaft_2sp, 'GGTcooling1', None, None,
                                    ...
                                    0.8,    # fraction of bleed flow taken
                                    1.0,    # dP fraction (extend to which expansion contributes to turbine power)
                                    0.8,    # Wc fraction (fraction of cooling flow Wc taking up turbine flow cross area
                                    0.2),     # pumpin radius (only > 0 for rotor blades)
    ...
    turbine_gg  =  TTurbine(turboshaft_2sp,
                             'GGT'   , 'turbimap.map', None, 4, 45, '_gg', 6800, 0.83 , 1, 0.6, 0.99, 'GG', GGTcoolingflows)
    ...
    turbine_PT  =    TTurbine(turboshaft_2sp,    # owning system model object
                             ...
                             0.98,              # design mechanical efficiency
                             'PT',              # turbine type : power turbine delivering shaft power
                             PTcoolingflows     # optional cooling flow list
                             )
    ...
    duct1    = TDuct(       turboshaft_2sp,     # owning system model object
                            ...
                            0.98                # design pressure ratio, use to specify rel. pressure loss ploss (PR = (1 - ploss)/Pin)
                            )
